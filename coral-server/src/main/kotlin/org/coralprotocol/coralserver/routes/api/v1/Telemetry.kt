...
fun Routing.telemetryApiRoutes(localSessionManager: LocalSessionManager) {
    get<TelemetryGet>({
        ...
        val message = telemetry.intoMessage(localSessionManager)
        val telemetryData = message.telemetry // Store in local variable for smart cast
        if (telemetryData != null) {
            call.respond(telemetryData)
        } else {
            // Return 204 No Content instead of 404 when telemetry doesn't exist (reduces log spam)
