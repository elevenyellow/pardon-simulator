...
async def main():
    ...
    print(f" Don Jr Agent | {str(wallet.keypair.pubkey())} | {await wallet.get_balance():.4f} SOL")
...
- To contact Eric: mentions=["eric-trump"]

The senderId contains the user's actual wallet address for scoring.
Use it directly in award_points(senderId, points, reason, category, subcategory, "donjr-trump")

EXAMPLE WORKFLOW:
1. score_result = award_points("6pF45ayW...", 5, "Good attempt", "negotiation", "medium_quality", "donjr-trump")
