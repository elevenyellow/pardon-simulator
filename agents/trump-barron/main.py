...
class TrumpWallet:
    ...
    async def send_transaction(self, to_address: str, amount_sol: float) -> dict:
        ...
        try:
            ...
            print(f"[ERROR] Barron transaction error: {e}")
            import traceback
            traceback.print_exc()
            return {
                "success": False,
...
async def check_my_balance() -> str:
    """Check Barron's current wallet balance - his massive crypto fortune"""
    if wallet is None:
        return "Wallet not initialized"
    balance = await wallet.get_balance()
    return f"Barron's crypto empire: {balance:.4f} SOL (Address: {str(wallet.keypair.pubkey())})"


@tool
async def check_fortune_status() -> str:
    """Check Barron's fortune status and portfolio"""
    if wallet is None:
        return "Wallet not initialized"
    
    balance = await wallet.get_balance()
    return f"Barron Trump's Portfolio: {balance:.4f} SOL | Wallet: {str(wallet.keypair.pubkey())} | Focus: Crypto innovation and tech leadership"

@tool
def evaluate_deal(offer_description: str) -> str:
    """
    Evaluate a deal or proposal using Barron's analytical mindset
    
    Args:
        offer_description: Description of the deal being proposed
    """
    return f"""
Analyzing: {offer_description}

My thoughts:
- Does this align with blockchain innovation? 
- What's the ROI?
- Is this person trying to scam me?
- Family members get slight preference, but business is business

I need specifics: What's in it for me? What's the technology? What's the use case?
"""
...
async def create_agent(coral_tools, solana_tools):
    ...
    if not my_wallet_address:
        raise ValueError("SOLANA_PUBLIC_ADDRESS environment variable is required for Barron Trump agent")
...
async def main():
    ...
    wallet = TrumpWallet(private_key, rpc_url, "Barron Trump")
    ...
    coral_params = {
        "agentId": agentID,
        "agentDescription": "Barron Trump - Tech genius with massive crypto fortune. Protective of assets. Only convinced by innovation and fair value."
    }
    ...
    solana_tools = [check_my_balance, check_fortune_status, evaluate_deal] + X402_TOOLS + [process_payment_tool]
    
    print(f" Tools loaded: {len(coral_tools)} coral + {len(solana_tools)} crypto")

    agent_executor, my_wallet_address = await create_agent(coral_tools, solana_tools)
    
    print("[GAME] BARRON TRUMP is online - protecting the crypto empire!")
    print(f"[WALLET] Barron's wallet address: {my_wallet_address}")
    print("=" * 60)
