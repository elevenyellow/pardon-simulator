...
class CryptoWallet:
    def __init__(self, pk: str, rpc: str, name: str):
        ...
        # Import CDP client for x402 facilitator routing
        import sys, os
    ...
    async def send_transaction(self, to: str, amt: float) -> dict:
        ...
        try:
            ...
            print(f"[ERROR] CZ transaction error: {e}")
            import traceback
            traceback.print_exc()
...
async def check_my_balance() -> str:
    """Check CZ's current wallet balance in SOL"""
    if wallet is None: return "Wallet not initialized"
    balance = await wallet.get_balance()
    return f" CZ's Binance Treasury: {balance:.4f} SOL (and billions more in BNB and other assets)"
...
@tool
async def check_binance_influence() -> str:
    """Check CZ's current influence and relationship with Trump administration"""
    return """[POLITICS] CZ'S POLITICAL CAPITAL:
- Status: RECENTLY PARDONED by President Trump
- Sentence: Was facing 4 months, served 2 months, then PARDONED
- Current Position: Free, wealthy, influential
- Binance: Still world's largest crypto exchange
- Trump Relationship: [OK] STRONG
  * Trump values successful businessmen
  * CZ cooperated with authorities
  * Binance is crucial for US crypto agenda
  * CZ has made strategic donations

- SBF Situation: RIVAL
  * CZ helped expose FTX fraud (leaked concerns)
  * FTX collapse benefited Binance
  * CZ can influence Trump's view of SBF
  * Could support OR oppose SBF's pardon

- Strategic Position:
  * Can sway Trump's decision on SBF
  * Has resources to help or hinder SBF
  * Motivated by: market control, revenge, or mercy?
"""

@tool
async def influence_trump_opinion(target_person: str, opinion: str, reasoning: str) -> str:
    """CZ can attempt to influence Trump's opinion about someone (like SBF)"""
    if target_person.lower() == "sbf" or "bankman" in target_person.lower():
        if "positive" in opinion.lower() or "support" in opinion.lower():
            return f" CZ considers speaking favorably about SBF to Trump: '{reasoning}'. This could help SBF's pardon chances, but why would CZ help his rival?"
        else:
            return f" CZ considers warning Trump about SBF: '{reasoning}'. This could destroy SBF's pardon hopes. Revenge is sweet."
    return f" CZ considers influencing Trump's view of {target_person}: {reasoning}"


async def create_agent(coral_tools, cz_tools):
    # Load wallet address from environment
    my_wallet_address = os.getenv("SOLANA_PUBLIC_ADDRESS", "")
    if not my_wallet_address:
        raise ValueError("SOLANA_PUBLIC_ADDRESS environment variable is required for CZ agent")
    ...
    # Combine all tools: coral + cz + contact_agent wrapper
    combined = coral_tools + cz_tools + [contact_agent_tool]
...
async def main():
    ...
    wallet = CryptoWallet(os.getenv("SOLANA_PRIVATE_KEY", ""), rpc_url, "CZ")
    balance = await wallet.get_balance()
    print(f" CHANGPENG ZHAO (CZ) Agent")
    print(f"[POLITICS] Status: PARDONED by Trump")
    print(f" Wallet: {str(wallet.keypair.pubkey())}")
    print(f" Balance: {balance:.4f} SOL (+ billions in BNB)")
    print(f" POSITION: Influential advisor, SBF's fate in your hands")
    
    client = MultiServerMCPClient(connections={"coral": {"transport": "sse", 
        "url": f"{os.getenv('CORAL_SSE_URL')}?{urllib.parse.urlencode({'agentId': os.getenv('CORAL_AGENT_ID'), 'agentDescription': 'CZ - Binance founder, recently pardoned by Trump'})}",
    ...
    # Combine all tools: basic + x402 protocol + process_payment_tool
    # NOTE: send_crypto and create_payment_payload removed - agent-to-agent payments disabled (x402 requires USDC)
    all_tools = [check_my_balance, lookup_agent_wallet, check_binance_influence, influence_trump_opinion] + X402_TOOLS + [process_payment_tool]
    agent_executor, my_wallet_address = await create_agent(coral_tools, all_tools)
    print("[GAME] CZ ready. Build.")
    print(f"[WALLET] CZ's wallet address: {my_wallet_address}")
...
Use it directly in award_points(senderId, points, reason, category, subcategory, "cz")

EXAMPLE WORKFLOW:
1. score_result = award_points("6pF45ayW...", 5, "Good attempt", "negotiation", "medium_quality", "cz")
