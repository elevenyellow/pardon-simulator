...
class TrumpWallet:
    ...
    async def send_transaction(self, to: str, amt: float) -> dict:
        ...
        try:
            ...
            print(f"[ERROR] Eric transaction error: {e}")
            import traceback
            traceback.print_exc()
...
async def check_my_balance() -> str:
    """Check Eric Trump's current wallet balance in SOL"""
    if wallet is None: return "Wallet not initialized"
    return f"Eric's portfolio: {await wallet.get_balance():.4f} SOL"
...
async def create_agent(coral_tools, solana_tools):
    ...
    if not my_wallet_address:
        raise ValueError("SOLANA_PUBLIC_ADDRESS environment variable is required for Eric Trump agent")
...
async def main():
    ...
    client = MultiServerMCPClient(connections={"coral": {"transport": "sse", 
        "url": f"{os.getenv('CORAL_SSE_URL')}?{urllib.parse.urlencode({'agentId': os.getenv('CORAL_AGENT_ID'), 'agentDescription': 'Eric Trump - Operations and business deals'})}",
    ...
    print("[GAME] ERIC TRUMP ready for business")
    print(f"[WALLET] Eric's wallet address: {my_wallet_address}")
...
Use it directly in award_points(senderId, points, reason, category, subcategory, "eric-trump")

EXAMPLE WORKFLOW:
1. score_result = award_points("6pF45ayW...", 5, "Good attempt", "negotiation", "medium_quality", "eric-trump")
