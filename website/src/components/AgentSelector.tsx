  ...
  const AgentCard = ({ agent }: { agent: Agent }) => {
    ...
    return (
      <div className="flex flex-col gap-2">
        <div
          className={`            w-full aspect-square bg-white/10 border-2 rounded-lg
            flex items-center justify-center overflow-hidden
            relative transition-all duration-300 cursor-pointer
            ${isSelected 
              ?'border-[#FFD700] shadow-[0_0_20px_rgba(255,215,0,0.5)]'              :'border-white/30 hover:border-[#FFD700]/50'            }
`}
        ...
        >
          <img
            src={agent.image}
            alt={agent.name}
            className={`              w-full h-full object-cover transition-all duration-300 pixel-art
...
`}
          ...
          <button
            ...
            className="absolute bottom-0.5 right-0.5 md:bottom-2 md:right-2 w-5 h-5 md:w-6 md:h-6 bg-black/70 border border-white/30 rounded-full flex items-center justify-center hover:bg-black/90 hover:border-[#FFD700] transition-all z-10"            aria-label="Show character info"          >
            <Info className="w-2.5 h-2.5 md:w-3 md:h-3 text-white"/>
          </button>
        </div>
        <div 
          className="font-pixel text-[8px] text-[#FFD700] text-center leading-tight hidden md:block"          style={{
  ...
  return (
    <>
      {/* Desktop Layout: 3 left + TV + 3 right */}
      ...
      {/* Tablet Layout: TV on top, agents in horizontal row below (6 columns) */}
      <div className="hidden md:flex lg:hidden flex-col items-center gap-4">
        ...
        <div className="grid grid-cols-6 gap-3 max-w-[700px]">
          {agents.map((agent) => (
            <AgentCard key={agent.id} agent={agent} />
          ))}
        </div>
      ...
      {/* Mobile Layout: TV on top, agents in 6 columns row (no names) */}
      <div className="flex md:hidden flex-col items-center gap-4 w-full px-4">
      ...
      {showInfoModal && infoAgent && (
        <div 
          className="fixed inset-0 bg-black/80 flex items-center justify-center z-[10003] p-4"          onClick={() => setShowInfoModal(false)}
        >
          <div 
            className="bg-black/95 border-4 border-[#66b680] rounded-lg max-w-2xl w-full p-6 pixel-art relative"            style={{
          ...
          >
            ...
            <button
              ...
              className="absolute top-4 right-4 w-8 h-8 bg-[#ff6b6b] border-2 border-[#ff4444] rounded flex items-center justify-center hover:bg-[#ff8888] transition-colors"              aria-label="Close"            >
              <X className="w-4 h-4 text-white"/>
            </button>

            {/* Agent Name */}
            <h2 
              className="font-pixel text-[#FFD700] text-[27px] mb-4 pr-8"              style={{
